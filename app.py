...
# Custom Bright CSS for high visibility and glowing effects
st.markdown('''
    <style>
    .stApp {
...
def train_ai_model(df):
    ...
    return model, le_city, le_crop, averages, features

df = load_and_clean_data()
model, le_city, le_crop, averages, model_features = train_ai_model(df)
    ...
    if st.button("Check Market Price", use_container_width=True):
        ...
        if pd.isna(fair_value):
            ...
            with col_stat:
                ...
                diff = ((input_retail_price - fair_value) / fair_value) * 100
                if diff > 15:
                    st.markdown(f"<div class='metric-card warning-card'><h4>🚨 Too Expensive!</h4><h2 style='color:#ff3d00;'>{diff:.0f}% Extra Price</h2><p>The shopkeeper is overcharging you.</p></div>", unsafe_allow_html=True)
                elif diff < -5:
                    st.snow()
                    st.markdown(f"<div class='metric-card'><h4>✅ Great Deal!</h4><h2 style='color:#00e676;'>{abs(diff):.0f}% Cheaper</h2><p>You are getting a very good price.</p></div>", unsafe_allow_html=True)
                else:
                    st.balloons()
                    st.markdown(f"<div class='metric-card' style='border-left: 6px solid #ffc107;'><h4>⚖️ Fair Price</h4><h2 style='color:#ffc107;'>Normal</h2><p>The price is correct according to market.</p></div>", unsafe_allow_html=True)
...
with tab2:
    ...
    if 'month_name' in df.columns:
        ...
        with fc1:
            ...
            farmer_kg = st.number_input("Total KGs you will sell:", min_value=40, value=1000, step=40)
        ...
        fig_line = px.area(f_data, x='month_name', y='Profit', markers=True, template="plotly_dark")
...
# ---------------------------------------------------------
# TAB 3: GOVERNMENT POLICY TESTER (FIXED LOGIC)
# ---------------------------------------------------------
with tab3:
    ...
    with g2:
        ...
        # --- ECONOMIC LOGIC ENFORCER ---
        diff_fuel = sim_fuel - averages['fuel']
        diff_inf = sim_inflation - averages['inflation']
        diff_temp = sim_temp - averages['temp']
        
        # Force logic: Positive change = Positive Price Increase
        logic_penalty = (diff_fuel * 0.9) + (diff_inf * 1.5) + (diff_temp * 1.2)
        ...
        fig_bar.update_layout(barmode='group', paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color='white'), height=300)
